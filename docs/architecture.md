# Collab Canvas Architecture

## Overview

Collab Canvas is a real-time collaborative canvas application with AI-powered natural language control. Users can create, edit, and manipulate multiple shape types (rectangles, circles, lines, text) on a shared canvas through direct manipulation, AI commands, or collaborative commenting. All changes sync instantly across connected clients with production-grade security.
...
- **Testing:** Vitest, React Testing Library (395 tests, 70%+ coverage)
...
> 
> **Code Structure:** For project organization, see the [README](../README.md).
...
**New Shape Types:**
- Circles, Lines, Text (with formatting: bold, italic, font size)
- All shapes support: selection, drag, resize, rotate, color, delete
- Shape-specific components: `Circle.tsx`, `Line.tsx`, `Text.tsx`
...
- Google Sign-In (primary), user profiles with initials/avatars
- Firebase Auth Triggers: `onAuthUserCreated`, `onAuthUserDeleted`  
- Production database rules: field-level validation, creator-only modification
- `UserProfilesContext` for efficient profile caching

**AI Enhancements:**
- AI supports all shape types (circles, lines, text)
...
- `UserProfilesContext` - User profile caching
- Updated `canvasService.ts` - Generic shape operations

### Remaining Future Enhancements

1. Undo/redo functionality with command history
2. Workspace/project management
3. Export to image/SVG
4. Voice input for AI commands
5. Real-time collaborative text editing
6. Shape grouping/ungrouping
7. Layers panel
8. Custom shape colors (beyond preset 3)
9. Infinite canvas with virtualization
